...
import pytest
from inkex import Circle, Group, Rectangle, SvgDocumentElement, TextElement, Transform, Use
...
class TestCloneElement:
    def assert_angle_almost_equal(self, a, b) -> None:
        ...
        assert a % 180 == pytest.approx(b % 180, abs=1e-4)

    def assert_transform_close(
        self,
        actual: Transform,
        expected: Transform,
        decimals: int = 4,
    ...
    def test_not_clone(self) -> None:
        root: SvgDocumentElement = svg()
        rect = root.add(Rectangle(attrib={"width": "10", "height": "10", INKSTITCH_ATTRIBS["angle"]: "30"}))
        use = root.add(Use(attrib={INKSTITCH_ATTRIBS["clone"]: "false"}))
        use.href = rect
    ...
    def test_basic(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), 30)

    def test_hidden_cloned_elements_not_embroidered(self) -> None:
        root = svg()
        g = root.add(Group())
        g.add(Rectangle(attrib={INKSCAPE_LABEL: "NotHidden", "width": "10", "height": "10"}))
        g.add(Rectangle(attrib={INKSCAPE_LABEL: "Hidden", "width": "10", "height": "10", "style": "display:none"}))
        ...
        with clone.clone_elements() as elements:
            assert len(elements) == element_count()
            assert elements[0].node.get(INKSCAPE_LABEL) == "NotHidden"
    ...
    def test_angle_rotated(self) -> None:
        ...
        with clone.clone_elements() as elements:
            assert len(elements) == element_count()
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), 10)
    ...
    def test_angle_flipped_rotated(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -50)
    ...
    def test_angle_non_uniform_scale(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -55)
    ...
    def test_angle_inherits_down_tree(self) -> None:
        ...
        rect = g1.add(Rectangle(attrib={"width": "10", "height": "10", INKSTITCH_ATTRIBS["angle"]: "30"}))
        g2 = root.add(Group())
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), 29)
    ...
    def test_angle_not_applied_twice(self) -> None:
        ...
        root: SvgDocumentElement = svg()
        rect = root.add(Rectangle(attrib={"width": "10", "height": "10", "style": "stroke: skyblue; fill: red;"}))
        use = root.add(Use())
        use.href = rect
        ...
        clone = Clone(use)
        with clone.clone_elements() as elements:
            assert len(elements) == element_count() + 1  # One for the stroke, one for the fill, one for the SewStack
            assert elements[0].node == elements[1].node
    ...
    def test_angle_not_applied_to_non_fills(self) -> None:
        ...
        root: SvgDocumentElement = svg()
        rect = root.add(Rectangle(attrib={"width": "10", "height": "10", "style": "stroke: skyblue; fill-opacity: 0;"}))
        use = root.add(Use())
        use.href = rect
        use.set("transform", Transform().add_rotate(30))
        ...
        clone = Clone(use)
        with clone.clone_elements() as elements:
            assert len(elements) == element_count()  # One for the stroke, one for the fill, one for the SewStack
            assert elements[0].get_param("angle", None) is None  # Angle as not set, as this isn't a fill
    ...
    def test_transform_inherits_from_cloned_element(self) -> None:
        ...
        with clone.clone_elements() as elements:
            assert len(elements) == element_count()
            self.assert_transform_close(elements[0].node.composed_transform(), Transform().add_translate((5, 10)).add_scale(2, 2))
    ...
    def test_transform_inherits_from_tree(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_transform_close(
                ...
                Transform().add_translate((5, 10)).add_translate((0, 5)).add_rotate(5).add_scale(2, 2),  # use  # g1  # rect
                5,
            )
    ...
    def test_transform_inherits_from_tree_up_tree(self) -> None:
        ...
        clone = Clone(use)
        with clone.clone_elements() as elements:
            assert len(elements) == element_count() * 2  # FillStitch, SewStack, FillStitch, SewStack
            self.assert_transform_close(
                elements[0].node.composed_transform(),
                Transform()
                .add_translate((1, 2))
            ...
            self.assert_transform_close(
                ...
                Transform().add_translate((1, 2)).add_scale(0.5, 1).add_translate((5, 10)).add_translate((0, 5)).add_rotate(5),  # g2  # use  # g1
                5,
            )
    ...
    def test_clone_fill_angle_not_specified(self) -> None:
        ...
        clone = Clone(use)
        assert clone.clone_fill_angle is None
    ...
    def test_clone_fill_angle(self) -> None:
        ...
        clone = Clone(use)
        assert clone.clone_fill_angle == 42

        with clone.clone_elements() as elements:
            assert len(elements) == element_count()
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), 42)
    ...
    def test_recursive_uses(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            assert len(elements) == element_count() * 2
            assert type(elements[0]) == FillStitch
            assert elements[0].node.tag == SVG_RECT_TAG
            self.assert_transform_close(
                elements[0].node.composed_transform(), Transform().add_translate((0, 30)).add_translate(0, 20).add_scale(0.5, 0.5)  # u3  # u2
            )

            assert type(elements[element_count()]) == FillStitch
            assert elements[element_count()].node.tag == SVG_RECT_TAG
            self.assert_transform_close(
                elements[element_count()].node.composed_transform(),
                Transform().add_translate((0, 30)).add_translate((0, 20)).add_scale(0.5, 0.5).add_translate((20, 0)),  # u3  # u2  # u1
            )
    ...
    def test_recursive_uses_angle(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -20)
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -27)
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -37)
    ...
    def test_recursive_uses_angle_with_specified_angle(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            self.assert_angle_almost_equal(element_fill_angle(elements[0]), -7)
    ...
    def test_copies_directly_attached_commands(self) -> None:
        ...
        with clone.clone_elements() as elements:
            ...
            assert cmd_orig.target_point[0] + 10 == pytest.approx(cmd_clone.target_point[0], abs=1e-4)
            assert cmd_orig.target_point[1] + 10 == pytest.approx(cmd_clone.target_point[1], abs=1e-4)
    ...
    def test_trim_after(self) -> None:
        root: SvgDocumentElement = svg()
        rect = root.add(Rectangle(attrib={"width": "10", "height": "10", INKSTITCH_ATTRIBS["trim_after"]: "true"}))
        ...
        assert len(stitch_groups) > 0
        assert stitch_groups[-1].trim_after
    ...
    def test_stop_after(self) -> None:
        root: SvgDocumentElement = svg()
        rect = root.add(Rectangle(attrib={"width": "10", "height": "10", INKSTITCH_ATTRIBS["stop_after"]: "true"}))
        ...
        stitch_groups = clone.embroider(None)
        assert len(stitch_groups) > 0
        assert stitch_groups[-1].stop_after
