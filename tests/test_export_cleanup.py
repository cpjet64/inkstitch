...
class DummyTempDir:
    """Simple TemporaryDirectory stand-in with deterministic path."""
...
def test_zip_effect_cleans_generated_files_when_zipping_fails():
    ...
    with patch("lib.extensions.zip.stitch_groups_to_stitch_plan", return_value=object()), \
            ...
            patch("lib.extensions.zip.tempfile.TemporaryDirectory", return_value=DummyTempDir("/tmp/zip-workdir")), \
    ...
    assert remove_file.call_args_list == [call("/tmp/export.zip")]
...
def test_batch_lettering_cleans_generated_files_when_zipping_fails():
    ...
    with patch("lib.extensions.batch_lettering.tempfile.TemporaryDirectory", return_value=DummyTempDir("/tmp/batch-workdir")), \
    ...
    assert remove_file.call_args_list == [call("/tmp/batch.zip")]
